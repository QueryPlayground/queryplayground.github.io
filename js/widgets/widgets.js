...
function horizontal_bar_chart(item, url) {
    ...
    var linedata = {
        ...
        datasets: [{
            ...
            data: _.map(_.pluck(data, "number_of_requests"), function(num) {
                return parseInt(num)
            })
        }]
    }
...
function handleSODAPlayground() {
    $.each($('.sodaplayground'), function(item) {
        ...
        try {
            ...
            var copyHtml = '<i class="fa fa-clipboard copythis" data-copy-this="' + encodeURI(htmlToCopy) + '"></i><span class="copythiscontainer"></span>';
